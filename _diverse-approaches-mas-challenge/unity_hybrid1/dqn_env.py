...
class TruckEnv:
    ...
    def __init__(self, cfg):
        ...
        # Config knobs for deadlock avoidance
    ...
    # dqn_env.py (inside TruckEnv class)
    ...
    def _apply_collisions(self, rewards):
        # still log penalties/yields during warm-up? -> keep it off to prevent lock
    ...
    def _car_following_speed_scale(self, t_self, all_trucks):
        # Warm-up: skip car-following entirely
        if self.current_step < self._warmup_ticks:
            return 1.0

        # Ignore car-following in a small bubble around the depot (avoid staging deadlocks)
        if dist(t_self.pos, self.city.depot) <= self._depot_unblock_r:
            return 1.0

        cone_deg = float(self.cfg.get("FORWARD_CONE_DEG", 35.0))
        ...
        for t in all_trucks:
            ...
            if (vx*dx + vy*dy) / d < cos_cut:
                continue
            # ignore someone who is essentially alongside (small lateral but behind/no forward progress)
        ...
        if best_d <= safe_stop:
            return 0.0
        if best_d <= safe_slow:
            return (best_d - safe_stop) / (safe_slow - safe_stop) * 0.5
        return 1.0
    ...
    def step(self, actions):
        ...
        # 3) action masking (safety + purposeful idle)
        ...
        for idx, truck in enumerate(self.sim.trucks):
            ...
            # Yield enforcement still applies
            if self._yield_cd.get(truck.tid, 0) > 0:
                a = 4

            # Return-to-depot: set route then MOVE
        ...
        # 3b) car-following speed scaling (keep your logic, but no reward effects)
        for t in self.sim.trucks:
            scale = self._car_following_speed_scale(t, self.sim.trucks)
            has_route = bool(t.route_pts) or (t.target is not None)
            if has_route:
                scale = 1.0 if self.current_step < self._warmup_ticks else max(scale, self._min_speed_scale)
            t.speed_scale = float(min(max(0.0, scale), 1.0))
        ...
        # 5) safety shaping (just logs yield + events; € is accounted via _step_cost_eur)
        self._apply_collisions(rewards=[0.0]*self.n_agents)  # ignore returned shaping

        # --- NEW: compute team reward from € delta
        cost_now = self._step_cost_eur()
        r_team = -(cost_now - cost_prev)
        rewards = [r_team] * self.n_agents

        # 7) log + advance time (unchanged)
        self._log_frame()
        self.sim.t += dt
        self.current_step += 1
