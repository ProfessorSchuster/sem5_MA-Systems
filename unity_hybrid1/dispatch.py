...
def _avoided_overflow_eur(cfg: dict, now: float, eta_s: float, due_s: float) -> float:
    ...
    if eta_s >= due_s:
        return 0.0
    # Heuristic: if you arrive X seconds before due, you avoid a fraction of expected risk.
    # Use a soft horizon to avoid binary jumps.
    horizon = float(cfg.get("URGENCY_HORIZON_S", 600.0))
...
def auction(bins: List[BinObj], trucks: List[Truck], now: float, cfg: dict, plan_route: Callable):
    ...
    inflow_rate = (cfg["BIN_FILL_PER_STEP"][0] + cfg["BIN_FILL_PER_STEP"][1]) / 2.0
    ...
    defer_thr   = float(cfg.get("DEFER_FILL_FRAC", 0.0))
    ...
    def truck_bin_cost(t: Truck, b: BinObj) -> float:
        ...
        # If the bin is barely filled and allow defer, it's often cheaper to skip now.
        fill_frac = b.fill / max(1, b.capacity)
        defer_bias = 0.0
        if fill_frac < defer_thr:
            # Encourage waiting (positive cost bump)
            defer_bias = travel_eur * 0.5  # gentle nudge to delay
        credit = _avoided_overflow_eur(cfg, now, now + eta_s, due_s)
        # Effective cost is travel cost minus avoided fee (credit).
    ...
    while free and len(taken) < len(cand):
        ...
        # remember assignment stats on the bin
